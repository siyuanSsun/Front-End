...
let initData = function () {
    for (let i = 0; i < sourceData.length; i++) {
        ...
        productCol.innerHTML = dataPiece['product'];
        let regionCol = document.createElement("th");
        regionCol.innerHTML = dataPiece['region']
        tr.appendChild(productCol);
        tr.appendChild(regionCol);

        for (let j = 0; j < dataPiece['sale'].length; j++) {
            let monthCol = document.createElement("th");
            monthCol.innerHTML = dataPiece['sale'][j];
            tr.appendChild(monthCol);
        }
